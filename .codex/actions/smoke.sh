...
if ! curl -sf "$BASE_URL/healthz" >/dev/null; then
  ...
  "$VENV_BIN/uvicorn" core.nexus_core.app:app --host 127.0.0.1 --port "$PORT" >"$ROOT_DIR/.nexus/logs/smoke-api.log" 2>&1 &
  SERVER_PID=$!
  STARTED_SERVER=1
...
"$VENV_BIN/python" scripts/smoke_api.py --base-url "$BASE_URL" "$@"
